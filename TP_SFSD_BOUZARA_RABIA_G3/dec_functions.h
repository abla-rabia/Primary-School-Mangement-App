#ifndef DEC_FUNCTIONS_H_INCLUDED
#define DEC_FUNCTIONS_H_INCLUDED
...
#define nb_names 260 //nombre de nom dans le fichier
#define max_index 50
...
/***  Index de l'archive  ***/ //un index non dense par bloc
...
//  Déclaration et Défintion du fichier Index de l'archive:
// --------------------------------------------------------

typedef struct enreg_indexA {
    int Id_min;
    int Id_max;
    float moy_min[n_ans];
    float moy_max[n_ans];
}enreg_indexA;
typedef struct tbloc_TOFA{
    enreg_indexA tab[max_tof];
}tbloc_TOFA;
typedef struct t_enteteA{
    int nb_bloc;
    int nb_enreg;
}t_enteteA;
typedef struct TOFA{
...
TOFA *f_index_A;//fichier d'index de l'archive
//--------------------------------------------
...
int nb; // nb d'elements dans la table d'index des eleves d'une classe
...
/*---------------------------------------------------------------------------------------------------------------------*/
//---------------------------------------
//La table utilisee dans l'insertion des nouveaux non prepas dans l'archive
int n_nonprep;
int non_prep[max_ins];
...
void affichage_Archive2(char name[max_str],int id);
...
bool insert(char nom_fichier[max_file], char classe[Tc],char nom[max_str],char prenom[max_str],char note[TeM],char genre,int not[TeM/2]);
...
void MAJ_Archive(char nom_archive[20]);
...
void Inserer_Archive(int id,int anne);
void lister_eleves(int moy , int durree);
void lister_eleves_annee(int moy,int annee);
void lister_eleves_annee_courante(int moy);
void Transfert(char nom_fich[max_file],char nom_archive[max_file],int cmp_annee);



void affiche_classe(char nom_fich[max_file],char classe[Tc]);


//fichier index archive et ses fonctions
...
void Chargement_indexA();


#endif // DEC_FUNCTIONS_H_INCLUDED
